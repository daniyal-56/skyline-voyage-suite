...
export function validateSignup(values: {
  ...
  const errors: ValidationResult<"name" | "email" | "phone" | "password" | "confirmPassword" | "acceptedTerms"> = {};
...
export function validatePayment(values: {
  ...
  const errors: ValidationResult<"cardholder" | "cardNumber" | "expiry" | "cvv" | "billingAddress"> = {};
  ...
  if (values.billingAddress.trim().length < 8) errors.billingAddress = "Billing address is required.";
