...
function PaymentPage() {
  ...
  const [values, setValues] = useState({ cardholder: user?.name || "", cardNumber: "", expiry: "", cvv: "", billingAddress: "" });
  const [errors, setErrors] = useState<Partial<Record<"cardholder" | "cardNumber" | "expiry" | "cvv" | "billingAddress", string>>>({});
          ...
          <div className="lg:col-span-2 order-2 lg:order-1">
            <div className="bg-card rounded-2xl p-6 sticky top-24" style={{ boxShadow: "var(--shadow-card)" }}>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Passenger</span><span className="text-foreground">{user?.name || "Guest Passenger"}</span></div>
            ...
            <div className="bg-card rounded-2xl p-6 lg:p-8" style={{ boxShadow: "var(--shadow-card)" }}>
              ...
              <form className="space-y-4" onSubmit={handleSubmit} noValidate>
                <div>
                  ...
                  <input className="w-full h-12 px-4 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/20 transition-all" placeholder="John Doe" value={values.cardholder} onChange={(event) => setValues((prev) => ({ ...prev, cardholder: event.target.value }))} aria-invalid={Boolean(errors.cardholder)} />
                  {errors.cardholder && <p className="mt-1.5 text-xs font-medium text-error">{errors.cardholder}</p>}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 h-12 px-4 rounded-xl border border-border bg-background focus-within:border-teal focus-within:ring-2 focus-within:ring-teal/20 transition-all">
                    ...
                    <input className="w-full text-sm bg-transparent focus:outline-none" placeholder="4242 4242 4242 4242" inputMode="numeric" value={values.cardNumber} onChange={(event) => setValues((prev) => ({ ...prev, cardNumber: event.target.value }))} aria-invalid={Boolean(errors.cardNumber)} />
                  </div>
                  {errors.cardNumber && <p className="mt-1.5 text-xs font-medium text-error">{errors.cardNumber}</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">Expiry Date</label>
                    <input className="w-full h-12 px-4 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/20 transition-all" placeholder="MM/YY" value={values.expiry} onChange={(event) => setValues((prev) => ({ ...prev, expiry: event.target.value }))} aria-invalid={Boolean(errors.expiry)} />
                    {errors.expiry && <p className="mt-1.5 text-xs font-medium text-error">{errors.expiry}</p>}
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">CVV</label>
                    <input className="w-full h-12 px-4 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/20 transition-all" placeholder="123" inputMode="numeric" value={values.cvv} onChange={(event) => setValues((prev) => ({ ...prev, cvv: event.target.value }))} aria-invalid={Boolean(errors.cvv)} />
                    {errors.cvv && <p className="mt-1.5 text-xs font-medium text-error">{errors.cvv}</p>}
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">Billing Address</label>
                  <input className="w-full h-12 px-4 rounded-xl border border-border bg-background text-sm focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/20 transition-all" placeholder="123 Main St, New York, NY" value={values.billingAddress} onChange={(event) => setValues((prev) => ({ ...prev, billingAddress: event.target.value }))} aria-invalid={Boolean(errors.billingAddress)} />
                  {errors.billingAddress && <p className="mt-1.5 text-xs font-medium text-error">{errors.billingAddress}</p>}
                </div>
