...
function SignupPage() {
  ...
  const [errors, setErrors] = useState<
    Partial<
      Record<"name" | "email" | "phone" | "password" | "confirmPassword" | "acceptedTerms", string>
    >
  >({});
        ...
        <div className="w-full max-w-md">
          ...
          <form className="space-y-4" onSubmit={handleSubmit} noValidate>
            <div>
              ...
              {errors.name && (
                <p className="mt-1.5 text-xs font-medium text-error">{errors.name}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.phone && (
                <p className="mt-1.5 text-xs font-medium text-error">{errors.phone}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.confirmPassword && (
                <p className="mt-1.5 text-xs font-medium text-error">{errors.confirmPassword}</p>
              )}
            </div>
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input
                type="checkbox"
                className="w-4 h-4 rounded border-border text-teal focus:ring-teal mt-0.5"
                checked={values.acceptedTerms}
                onChange={(event) =>
                  setValues((prev) => ({ ...prev, acceptedTerms: event.target.checked }))
                }
            ...
            {errors.acceptedTerms && (
              <p className="-mt-2 text-xs font-medium text-error">{errors.acceptedTerms}</p>
            )}
